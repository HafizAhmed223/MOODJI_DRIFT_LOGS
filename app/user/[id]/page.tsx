...
import {
  ...
  StatsPanel,
  SectionDivider,
} from "@/components/ui/narrative-panel";
...
import { DriftLogEntry, getMoodColor, formatDate } from "@/lib/drift-data";
import {
  CustomTooltip,
  chartConfigs,
  processTimelineData,
...
export default function UserProfile({
  ...
  const avgIntensity =
    userEntries.reduce((sum, e) => sum + e.creation.conflict_intensity, 0) /
    userEntries.length;
  const uniqueMoods = new Set(userEntries.map((e) => e.creation.mood_label))
    .size;
  const avgFrequency =
    userEntries.reduce((sum, e) => sum + e.creation.field.hz, 0) /
    userEntries.length;
    ...
    userEntries.length > 1
      ? Math.ceil(
          (new Date(latestEntry.created_at).getTime() -
            new Date(userEntries[0].created_at).getTime()) /
            (1000 * 60 * 60 * 24),
        )
      : 0;
            ...
            <div className="flex flex-col lg:flex-row lg:items-center gap-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-lg">
                    Personal Journey Visualization • {userEntries.length} Drift
                    Log {userEntries.length === 1 ? "Entry" : "Entries"}
                  </p>
                </div>
              </div>
              <div className="flex flex-wrap gap-4 lg:ml-auto">
                ...
                <Badge
                  style={{
                    backgroundColor:
                      getMoodColor(latestEntry.creation.mood_label) + "20",
                  }}
                  className="px-3 py-1"
                >
              ...
              <div className="lg:col-span-3">
                ...
                >
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart
                      data={timelineData}
                      margin={chartConfigs.timeline.margin}
                    >
                    ...
                    <div className="space-y-6">
                      <div className="flex flex-wrap gap-3">
                        <Badge
                          style={{
                            backgroundColor:
                              getMoodColor(selectedEntry.creation.mood_label) +
                              "30",
                          }}
                          className="px-3 py-1"
                        >
            ...
            >
              <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                {userEntries.map((entry, index) => (
                  <div
                    key={entry.id}
                    className={`p-4 rounded-lg border cursor-pointer transition-all duration-200 ${selectedEntry?.id === entry.id ? "border-celestial-aurora bg-celestial-aurora/5 shadow-md" : "border-celestial-aurora/20 hover:border-celestial-aurora/40 hover:bg-celestial-aurora/5"}`}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          <Badge
                            style={{
                              backgroundColor:
                                getMoodColor(entry.creation.mood_label) + "30",
                            }}
                            className="text-xs"
                          >
                          ...
                          {entry.final_payload && (
                            <Badge
                              variant="default"
                              className="text-xs bg-celestial-golden/20 text-celestial-golden"
                            >
                              Final
                            </Badge>
                          )}
