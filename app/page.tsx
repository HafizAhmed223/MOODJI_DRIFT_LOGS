          ...
          <div className="container mx-auto px-6 py-8">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
              <div className="space-y-2">
                <h1 className="text-4xl font-bold bg-gradient-to-r from-celestial-aurora to-celestial-plasma bg-clip-text text-transparent">Moodji Dashboard</h1>
                <p className="text-muted-foreground text-lg">Resonance Drift Log Monitoring Console</p>
              </div>
        ...
        <div className="container mx-auto px-6 py-8">
          {filteredUsers.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-muted-foreground">{searchTerm ? 'Try adjusting your search terms.' : 'No drift log entries available.'}</p>
            </div>
          ) : (
              ...
              {filteredUsers.map((user) => (
                ...
                >
                  ...
                  <CardHeader className="relative z-10 space-y-4">
                    ...
                    <div className="space-y-2">
                      <Badge className={`bg-${getMoodColor(user.latest_mood)}/20 text-${getMoodColor(user.latest_mood)} border-${getMoodColor(user.latest_mood)}/30 hover:bg-${getMoodColor(user.latest_mood)}/30`}>
                        {user.latest_mood}
                      </Badge>
                      <div className="text-sm text-muted-foreground">
                        ...
                        <div className="text-xs opacity-75">Last active: {formatDate(user.last_activity)}</div>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className="relative z-10">
                    <div className="space-y-3">
                      <div className="flex justify-between items-center text-sm">
                        ...
                        <span className="text-celestial-aurora font-medium">{Math.round(user.journey_completion)}%</span>
                      </div>
                      <div className="w-full bg-celestial-cosmos/30 rounded-full h-2 overflow-hidden">
                        <div className="bg-gradient-to-r from-celestial-aurora to-celestial-plasma h-full rounded-full transition-all duration-500 group-hover:from-celestial-plasma group-hover:to-celestial-quantum" style={{ width: `${user.journey_completion}%` }} />
                      </div>
                  ...
                  <div className="absolute inset-0 border border-celestial-aurora/0 group-hover:border-celestial-aurora/50 rounded-lg transition-all duration-300 pointer-events-none" />
                </Card>
              ))}
            </div>
          )}
